...
async def groq_json(messages: list[dict], max_retries: int) -> dict:
    """Make a rate-limited Groq JSON-mode call. Returns the parsed dict, or {}
    on quota death, persistent rate limiting or any other error."""
    global _tokens
    if is_quota_dead():
